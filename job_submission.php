...
<script type="text/javascript">
    function confirmClose(jobPauseAt, jobStartAgain) {
        if (jobPauseAt && !jobStartAgain) {
            alert("The job is currently paused. Please restart the job before closing it.");
...
?>
    ...
    ?>
                        ...
                        <div class="card card-block card-stretch">
                            ...
                            <?php
                                      if($log_name == 'Mahendran'){
                                    ?>
                                    <div class="right-end" style="margin: 0px 18px;">
                                  ...
                                  if($log_name== 'Mahendran'){
                                    ?>
                                        <th></th>
                                       <th scope="col" style="text-align:left">Client Name/<br> Name of the Job /<br>Job Type / <br>D.O.C/ <br>Layout Ideas / Color options </th>
                                   <?php } else{ ?>
                                    <th scope="col" style="text-align:left">Client Name/<br> Name of the Job /<br>Job Type /<br>D.O.C <br>Layout Ideas /  Color options </th>
                                    <?php }  ?>
                                    <?php
                                  ...
                                  if($log_name== 'Mahendran'){
                                    ...
                                    $dateDifference = floor(($completionDate - $today) / (60 * 60 * 24));
                                        ...
                                        ?>  
                                            ...
                                            <form method="POST" action="job_close.php">
                                            <td class="pr-0 ">
                                                <div class="custom-control custom-checkbox custom-control-inline">
                                             ...
                                             <td> 
                                                   <input type="hidden" name="job_id_closing" value="<?php echo $db_field2["id"]?>" >
                                               <?php
                                                  if ($dateDifference < 0) {
                                                    // Past date
                                                    echo '<span style="color: red;">'. $db_field2['clientname'] . '<br>'  . $db_field2['job'] . '<br>' . $db_field2['job_type'] .'<br>' . $db_field2['type_of_work'] . '<br>' . $db_field2['completion'] . '<br>' . $db_field2['start_time'].' - '  . $db_field2['time_comp']. '<br>'  .'Layout ' . $db_field2['layout_ideas'].' /'  . $db_field2['color_options'].'</span>';
                                                } elseif ($dateDifference == 0) {
                                                    // Today's date
                                                    echo '<span style="color: #03ae03;">'. $db_field2['clientname'] . '<br>'  . $db_field2['job'] . '<br>' . $db_field2['job_type'] .'<br>' . $db_field2['type_of_work'] . '<br>' . $db_field2['completion'] . '<br>'. $db_field2['start_time'].' - ' . $db_field2['time_comp']. '<br>'  .'Layout ' . $db_field2['layout_ideas'].' / '  . $db_field2['color_options']. '</span>';
                                                } else {
                                                    // Future date
                                                    echo $db_field2['clientname'] . '<br>'  . $db_field2['job'] . '<br>' . $db_field2['job_type'] . '<br>' . $db_field2['type_of_work'] . '<br>'. $db_field2['completion'] . '<br>' . $db_field2['start_time']. ' - ' . $db_field2['time_comp']. '<br>' .'Layout ' .  $db_field2['layout_ideas'].' / '  . $db_field2['color_options'];
                                                }
                                                    ?>
                                                </td>
                                                    ...
                                                    while ($row = mysqli_fetch_assoc($resultUpdated)) {
                                                        ...
                                                        if (empty($job_status) ) { ?>
                                                            <input value="Closed Completed" type="hidden" name="job_status">
                                                                <input class="button_submit" name="submit" onclick="return confirmClose();" type="submit" value="Close Now">
                                    ...
                                    $job_start_at =$db_field2["job_start_at"];
                                    $job_id_start =$db_field2["id"];
                                        ...
                                        ?>  
                                            ...
                                            <!-- <td class="pr-0 ">
                                                ...
                                                if (!empty($db_field2["start_time"]) && !empty($db_field2["time_comp"])) {
                                                    ...
                                                    if ($dateDifference < 0) {
                                                        // Past date
                                                        echo '<span style="color: red;">'. $db_field2['clientname'] . '<br>'  . $db_field2['job'] . '<br>' . $db_field2['job_type'] .'<br>'  . $db_field2['type_of_work'] .'<br>' . $db_field2['completion'] . '<br>' .  $twelveHourTime_starttime .' - '  . $twelveHourTime_endtime.'<br>' .'Layout ' . $db_field2['layout_ideas'].' /'  . $db_field2['color_options']. '</span>';
                                                    } elseif ($dateDifference == 0) {
                                                        // Today's date
                                                        echo '<span style="color: #03ae03;">'. $db_field2['clientname'] . '<br>'  . $db_field2['job'] . '<br>' . $db_field2['job_type'] .'<br>'  . $db_field2['type_of_work'] .'<br>' . $db_field2['completion'] . '<br>'.  $twelveHourTime_starttime .' - ' . $twelveHourTime_endtime. '<br>' .'Layout ' . $db_field2['layout_ideas'].' /'  . $db_field2['color_options']. '</span>';
                                                    } else {
                                                      
                                                        echo $db_field2['clientname'] . '<br>'  . $db_field2['job'] . '<br>' . $db_field2['job_type'] . '<br>' . $db_field2['type_of_work'] .'<br>' . $db_field2['completion'] . '<br>' .  $twelveHourTime_starttime . ' - ' . $twelveHourTime_endtime. '<br>' .'Layout ' . $db_field2['layout_ideas'].' /'  . $db_field2['color_options']  ;
                                                    }
                                                } 
                                              else {
                                                    if ($dateDifference < 0) {
                                                        // Past date
                                                         echo '<span style="color: red;">'. $db_field2['clientname'] . '<br>'  . $db_field2['job'] . '<br>' . $db_field2['job_type'] . '<br>' . $db_field2['completion'] . '<br>' . 'Time Not Mentioned' . '<br>' . 'Layout ' . $db_field2['layout_ideas'] . ' /' . $db_field2['color_options'] . '</span>'; 
                                                    } elseif ($dateDifference == 0) {
                                                        // Today's date
                                                        echo '<span style="color: #03ae03;">'. $db_field2['clientname'] . '<br>'  . $db_field2['job'] . '<br>' . $db_field2['job_type'] . '<br>' . $db_field2['completion'] . '<br>' .  'Time Not Mentioned' . '<br>' . 'Layout ' . $db_field2['layout_ideas'] . ' /' . $db_field2['color_options'] . '</span>';
                                                    } else {
                                                        // Future date
                                                        echo  $db_field2['clientname'] . '<br>'  . $db_field2['job'] . '<br>' . $db_field2['job_type'] . '<br>' . $db_field2['completion'] . '<br>' . ' TIme Not Mentioned ' . '<br>' . 'Layout ' . $db_field2['layout_ideas'] . ' /' . $db_field2['color_options']  ;
                                                    }
                                                }
                                                ?>
                                                </td>
                                    ...
                                    <td style="display:none;">
                                      ...
                                      <td class="text-center">
                                        ...
                                        $close_timed = $db_field2["close_time"];
                                        $members_jobs = $db_field2["members"];
                                        $job_pause_at = $db_field2["job_pause_at"];
                                        $job_start_again = $db_field2["job_start_again"];

                                        if (empty($job_start_at)) { ?>
                                            <input value="<?php echo $job_id_start; ?>" type="hidden" id="job_id_start">
                                            <input value="<?php echo $currentDateTime; ?>" type="hidden" id="job_start_time">
                                            <input value="<?php echo $members_jobs; ?>" type="hidden" id="members_jobs">
                                            <button id="startButton" class="button_start btn-danger" onclick="updateJobStatus('<?php echo $job_id_start; ?>', '<?php echo $currentDateTime; ?>', '<?php echo $members_jobs; ?>')">START</button>
                                        <?php } elseif (empty($job_pause_at)) { ?>
                                            <button id="pauseButton" class="button_pause btn-warning" onclick="pauseJob('<?php echo $job_id_start; ?>', '<?php echo $currentDateTime; ?>')">PAUSE</button>
                                        <?php } elseif (empty($job_start_again)) { ?>
                                            <button id="restartButton" class="button_start btn-success" onclick="restartJob('<?php echo $job_id_start; ?>', '<?php echo $currentDateTime; ?>')">Job Paused Wanna Restart</button>
                                        <?php } else { ?>
                                            <p class="button_start_p" style="color:#fff;">Job Restarted</p>
                                        <?php } ?>
                                    </td>
                                          ...
                                          $sqlUpdated = "SELECT * FROM jobcreate WHERE id = '$recordId'";
                                                    ...
                                                    while ($row = mysqli_fetch_assoc($resultUpdated)) {
                                                        ...
                                                        <input type="hidden" name="job_id_closing" value="<?php echo $db_field2["id"]?>" >
                                                           <input value="Closed Completed" type="hidden" name="job_status">
                                                                   <input class="button_submit btn-danger" name="submit" style="background:red;" type="submit" onclick="return confirmClose('<?php echo $job_pause_at; ?>', '<?php echo $job_start_again; ?>');" value="Close Now">
                                                        <?php }
                                                    } ?>
                                                        </form>
                                                </td>
    ...
    <!-- Wrapper End-->
   <script>
function updateJobStatus(job_id_start, job_start_time, members_jobs) {
    if (confirm("Are you sure you want to start the job?")) {
        // Create a FormData object
        let formData = new FormData();
        formData.append('job_id_start', job_id_start);
        formData.append('job_start_time', job_start_time);
        formData.append('member_jobs', members_jobs);  // Update this line

        // Send the AJAX request
        fetch('job_starts_at.php', {
            method: 'POST',
            body: formData
        })
        .then(response => response.text())
        .then(data => {
            alert(data); // For debugging, to check server response
...
function pauseJob(job_id_start, job_pause_time) {
    if (confirm("Are you sure you want to pause the job?")) {
        let formData = new FormData();
        formData.append('job_id_start', job_id_start);
        formData.append('job_pause_time', job_pause_time);
...
function restartJob(job_id_start, job_start_again_time) {
    if (confirm("Are you sure you want to restart the job?")) {
        let formData = new FormData();
        formData.append('job_id_start', job_id_start);
        formData.append('job_start_again_time', job_start_again_time);

        fetch('job_restart.php', {
            method: 'POST',
            body: formData
        })
        .then(response => response.text())
